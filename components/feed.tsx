...
export default function Feed({ onRefetchSignal }: { onRefetchSignal?: number }) {
      ...
      {loading && posts.length === 0 ? (
        ...
        >
          ...
          <div className="absolute inset-0 opacity-5">
            ...
            <div className="absolute bottom-4 right-4 w-16 h-16 rounded-full bg-blue-500 animate-pulse" style={{ animationDelay: "1s" }}></div>
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-32 h-32 rounded-full bg-purple-500 animate-pulse" style={{ animationDelay: "2s" }}></div>
          </div>
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-xl font-semibold text-textColor mb-2">Your feed is empty</h3>
  ...
  {posts.map((p: Post, index) => {
    ...
    return (
      <div
        key={p._id}
        className={`transform transition-all duration-300 ${isRemoving ? "opacity-0 -translate-y-4" : "opacity-100 translate-y-0"}`}
        style={{
      ...
      {!hasMore && posts.length > 0 && (
        <div className="text-center py-8">
          <div className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-background/50 border border-border/20 backdrop-blur-sm">
            ...
            <span className="text-subTextColor font-medium">You've reached the end</span>
            <div className="w-2 h-2 rounded-full bg-subTextColor/50"></div>
          </div>
        </div>
      )}
