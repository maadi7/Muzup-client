...
export default function PostCard({
  ...
  const emojiList = useMemo(() => {
    const sorted = [...localReactions].sort(
  ...
  const handleToggleReaction = async (emoji: string) => {
    ...
    if (idx !== -1) {
      const has = updated[idx].users?.some(
        (u) => u._id?.toString() === currentUserId?.toString()
      );
      if (has) {
        updated[idx].users = updated[idx].users!.filter(
          (u) => u._id?.toString() !== currentUserId?.toString()
        );
    ...
    try {
      ...
      const originallyReacted = (post.reactions ?? []).some(
        (r) =>
          r.emoji === emoji &&
          r.users?.some((u) => u._id?.toString() === currentUserId?.toString())
      );
      ...
      <header className="p-6 pb-4">
        <div className="flex items-start gap-4">
          ...
          <div className="relative">
            <Link href={`/Muzup/user/${post.user?._id}`}>
              ...
              <div className="relative">
                ...
                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-40 bg-background rounded-lg shadow-lg border border-border/10 z-50 p-2">
                    ...
                    {currentUserId &&
                      currentUserId === (post.user?._id as any) && (
                        <button
                    ...
                    {menuError && (
                      <div className="text-xs text-destructive px-3 pt-2">
                        {menuError}
                      </div>
                    )}
                  </div>
        ...
        {post.postUrl && (
          <div className="mb-4 rounded-2xl overflow-hidden bg-background/20">
            {post.postType?.toLowerCase?.() === "video" ? (
              ...
              <div className="relative">
                {!imageLoaded && (
                  <div className="absolute inset-0 bg-background/20 animate-pulse rounded-2xl"></div>
                )}
                <img
                  src={post.postUrl}
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            {quickEmojis.map((emoji) => {
              const count =
                localReactions.find((r) => r.emoji === emoji)?.users?.length ??
                0;
              ...
              return (
                <button
                  ...
                  className={`relative group flex items-center gap-1.5 px-3 py-2 rounded-full transition-all transform hover:scale-105 ${
                    active
                      ? "bg-muzupColor/15 text-muzupColor shadow-lg shadow-muzupColor/20"
                      : "bg-background/40 text-subTextColor hover:bg-background/60 hover:text-textColor"
                  ...
                  {count > 0 && (
                    <span
                      className={`text-xs font-medium ${
                        active ? "text-muzupColor" : "text-subTextColor"
                      }`}
                    >
                      {count}
                    </span>
